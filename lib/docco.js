(function() {
  ...
  destination = function(filepath, context) {
    ...
    return 'docs/' + base_path.replace(path.basename(base_path), '') + path.basename(filepath, path.extname(filepath)) + '.html';
  };
  ...
  parse_args = function(callback) {
    ...
    return exec("find " + relative_root + " -type f", function(err, stdout) {
      ...
      sources = stdout.split("\n").filter(function(file) {
        return file !== '' && path.basename(file)[0] !== '.' && languages[path.extname(file)];
      });
